// ============================================
// services/externalApiService.js - External Public API Client with Caching
// ============================================
...
/**
 * Helper to retrieve from cache or execute fetch and cache result
 */
const getCachedOrFetch = async (cacheKey, ttl, fetchFn) => {
    try {
        const redis = getRedisClient();
        const cached = await redis.get(cacheKey);
        if (cached) {
            console.log(`[Cache Hit] External API key: ${cacheKey}`);
            return JSON.parse(cached);
        }
    } catch (cacheError) {
        console.error(`[Cache Error] Failed to get key ${cacheKey}:`, cacheError.message);
    }
    ...
    try {
        ...
        console.log(`[Cache Miss & Write] Cached key ${cacheKey} for ${ttl}s`);
    } catch (cacheError) {
        console.error(`[Cache Error] Failed to set key ${cacheKey}:`, cacheError.message);
    }
...
export class ExternalApiService {
    ...
    static async fetchMotivationalQuote() {
        const cacheKey = 'external:motivational_quote';
    ...
    static async fetchDailyQuote() {
        const cacheKey = 'external:daily_quote';
        return getCachedOrFetch(cacheKey, 3600, async () => {
            try {
    ...
    static async fetchAdvice() {
        const cacheKey = 'external:advice';
        return getCachedOrFetch(cacheKey, 3600, async () => {
            try {
    ...
    static async fetchTrivia(amount = 5, difficulty = 'medium', category = 18) {
        const cacheKey = `external:trivia:${amount}:${difficulty}:${category}`;
    ...
    static async fetchDefinition(word) {
        ...
        const cacheKey = `external:definition:${encodeURIComponent(sanitizedWord)}`;
    ...
    static async fetchHackerNews(limit = 10) {
        const cacheKey = `external:tech-news:${limit}`;
        return getCachedOrFetch(cacheKey, 300, async () => {
            try {
    ...
    static async fetchActivity() {
        const cacheKey = 'external:activity';
        return getCachedOrFetch(cacheKey, 300, async () => {
            try {
